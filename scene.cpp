...
void display(void)
{
	...
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0.0f, 5.5f, 0.0f)); // Translate it down a bit so it's at the center of the scene
...
void init(void)
{
    ...
    glDepthFunc(GL_LESS);
	modelShader = new Shader("../shader/nanosuit.vs", "../shader/nansuit.frag");
    skybox->setup();

    cube->setup();
	nanomodel = new Model("../res/obj/cyborg/cyborg.obj");
